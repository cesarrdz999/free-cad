...
export default class Config extends Observable{
    ...
    set quantity(quantity){
        ...
        this._notifyHandlers('change','quantity');
    }
    ...
    get showStraightLineDialog(){
        let ssld = localStorage.getItem('showStraightLineDialog');

        if (ssld === null) {
            localStorage.setItem('showStraightLineDialog', true);
            return true;
        }

        return ssld && ssld !== 'false';
    }

    set showStraightLineDialog(value){
        localStorage.setItem('showStraightLineDialog', value);
    }
}