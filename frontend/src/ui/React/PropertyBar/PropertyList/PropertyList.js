...
class ToolsPanel extends React.PureComponent {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        if (prevState.text != this.props.fieldValues.text) {
            this.setState({text: this.props.fieldValues.text});
        }

        if (this.props.fieldValues.text == "" && this.show('text') && !this.state.desktop) {
            setTimeout(() => {
