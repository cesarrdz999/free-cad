...
export default class TwoLinesInNoShape extends Rule{
    ...
    getTwoLinesByDocument(document){
        ...
        for(let i = 0; i< shapes.length - 1; i++) {
            for (let j = i + 1; j< shapes.length; j++) {
                ...
                if(shapes[i].elements[0].typeName === 'Arc' && shapes[i].elements[0].incrementAngle==360 ||
                    shapes[j].elements[0].typeName === 'Arc' && shapes[j].elements[0].incrementAngle==360) {
                    continue;
                }

                if (shapes[i].elements[0].typeName !== 'Arc' && 
                    shapes[j].elements[0].typeName !== 'Arc' && 
                    shapes[i].elements[0].isOverlapping(shapes[j].elements[0])) {
                    continue;
                }
    ...
    createExtendLinesSolution() {
        ...
        if (redElements) {
            if (redElements[0].typeName !== 'Line' || redElements[1].typeName !== 'Line') {
                return null;
            }
