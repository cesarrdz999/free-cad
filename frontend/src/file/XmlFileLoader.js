...
export default class XmlFileLoader extends FileLoader{
            ...
            parser.onopentag = (tag)=> {
                ...
                switch (tag.name){
                    ...
                    case 'Method':
                        ...
                        break;
                    case 'QuantityOfParts':
                        container.resolve('config').quantity = Number.parseInt(tag.attributes.Value);
                }
                if(newElement){
                    if(height=='AirInside'){
     ...
     * @private
     */
    _createBendByTag(tag){
        let lineType=new Bend();
        lineType.processing = [this._createBendProcessingByTag(tag)];
