...
//import ZValueOfInnerShape from "./analyzers/rules/ZValueOfInnerShape";
...
export default class Application extends Observable{
            ...
            command.execute().then((res)=>{
                ...
                if(this._board){
                    if(command instanceof ElementModificationCommand){
                        ...
                        if(command instanceof ChangeTextCommand || command instanceof MoveElementsCommand){ //hack for performance. The nudge was too slow
                            ...
                            if (command.isReplacedElements()) {
                                ...
                                if(command instanceof IntersectElementsCommand){
                                    container.resolve('confirmChangeArcToSplinesDialog').modalNonWorkFeature(
                                        "Selected line doesn't intersect other lines.");
                                }
                            }
                        }
    ...
    openUrl(url) {
        ...
        if(!fileLoader){
            ...
            let response = Helper.Request.httpGet(url);
            fileLoader.convertDataToDocument(response).then(document => {
    ...
    import(file){
        ...
        let fileLoader = container.resolve('fileLoaderFactory', format);
        if(!fileLoader){
            container.resolve('confirmChangeArcToSplinesDialog').modalNonWorkFeature("Only *.dxf file format supported.");
        }else{
            fileLoader.load(file).then(data=>{
                this.openDocument(data);
            }).catch(e=>{
                container.resolve('confirmChangeArcToSplinesDialog').modalNonWorkFeature(e);
            });
        }
    }
    ...
    setTracingPaper(){
        Helper.openFile("image/png,image/jpeg").then((file)=>{
            if (!file) {
                return;
            }
    ...
    addElement(element){
        return new Promise((resolve, reject)=>{
            let command = new AddElementCommand(app.currentDocument, element, this.tracingPaper==null);
     ...
     */
    group(){
        return this.executeCommand(new GroupCommand(app.currentDocument, app.selectElements));
    }
     ...
     */
    ungroup(){
        return this.executeCommand(new UngroupCommand(app.currentDocument, app.selectElements));
    }
     ...
     */
    simplify(){
        return this.executeCommand(new SimplifyCommand(app.currentDocument, app.selectElements));
    }
    ...
    rotateSelected(angle, center=null){
        ...
        return this.executeCommand(new RotateElementsCommand(app.currentDocument, app.selectElements, angle, center));
    }

    deleteSelected(){
        if(this.board){
            if(this.board.tool && this.board.tool.name == "TracingPaper"){
                this.tracingPaper = null;
                this.board.tracingPaper=null;
                this._changeTool(this._getToolInstance('Pointer'));
                return;
            }
            this._changeTool(this._getToolInstance('Pointer'));
    ...
    setElementsHeight(height){
        return this.executeCommand(new ChangeElementsHeightCommand(app.currentDocument, app.selectElements, height));
    }
     ...
     */
    moveSelected(x,y){
        return this.executeCommand(new MoveElementsCommand(app.currentDocument, app.selectElements.slice(), x,y));
    }
     ...
     */
    mirrorSelected(axis){
        return this.executeCommand(new MirrorElementsCommand(this.currentDocument, this.selectElements, axis));
    }
    ...
    setElementsLineType(lineType){
        return this.executeCommand(new ChangeLineTypeCommand(app.currentDocument, app.selectElements, lineType));
    }
    ...
    copyMoveSelected(x,y){
        return this.pasteElements(this.selectElements, x, y);
    }
    ...
    setTextForSelectedElement(text){
        return this.executeCommand(new ChangeTextCommand(app.currentDocument, this.selectElements, text));
    }
    ...
    setFontSizeForSelectedElement(fontSize){
        return this.executeCommand(new ChangeFontSizeCommand(app.currentDocument, this.selectElements, fontSize));
    }
     ...
     */
    changeFont(fontName){
        return this.executeCommand(new ChangeFontCommand(app.currentDocument, this.selectElements, fontName));
    }
    ...
    intersectSelectedElements(){
        ...
        return this.executeCommand(new IntersectElementsCommand(this.currentDocument, this.selectElements));
    }
    ...
    setRadiusForSelectedElements(radius){
        return this.executeCommand(new ChangeArcsRadiusCommand(this.currentDocument, this.selectElements, radius));
    }
    ...
    setLineLengthElement(length){
        return this.executeCommand(new ChangeLineLengthCommand(this.currentDocument, this.selectElements, length));
    }
    ...
    setLineAngleElement(angle){
        return this.executeCommand(new ChangeLineAngleCommand(this.currentDocument, this.selectElements, angle));
    }
    ...
    setSelectedElementsSize(width, height, convertCircleToSplines=true){
        ...
        let cpVertival = ResizeElementsCommand.CONTROL_POINT_Y.top;
        let cpHorisontal = ResizeElementsCommand.CONTROL_POINT_X.right;

        if(Helper.Math.equals(width, oldWidth)){
            cpHorisontal=ResizeElementsCommand.CONTROL_POINT_X.center;
        }

        if(Helper.Math.equals(height, oldHeight)){
            cpVertival=ResizeElementsCommand.CONTROL_POINT_Y.center;
        }

        let command = new ResizeElementsCommand(this.currentDocument, this.selectElements, vector
                    , cpHorisontal, cpVertival, convertCircleToSplines);
    ...
    setArcAngles(startAngle, insideAngle){
        return this.executeCommand(new ChangeArcAngleCommand(this.currentDocument, this.selectElements, insideAngle, startAngle));
    }
    ...
    tangentsSelectedArcs(){
        return this.executeCommand(new TangentsArcsCommand(this.currentDocument, this.selectElements));
    }

    /**
     * @param {number} countParts
     * @return {Promise<boolean>}
     */
    divideSelectElement(countParts=2){
        return this.executeCommand(new DivideElementsCommand(this.currentDocument, Document.toSimpleListElements(this.selectElements), countParts));
    }
     ...
     */
    corner(){
        return this.executeCommand(new RoundCornerCommand(this.currentDocument, this.selectElements));
    }
    ...
    print(){
        ...
        windowContent += '<body style="overflow: hidden">';
    ...
    changeBendRadius(radius){
        return  this.executeCommand(new ChangeBendRadiusCommand(this.currentDocument, radius));
    }
