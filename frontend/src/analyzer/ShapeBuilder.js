...
export default class ShapeBuilder{ //todo: create an interface for declares the public methods
     ...
     * @return {Array.<Shape>}
     */
    buildShapes(withBendLikeSimpleElement = false, ignoreEngrave = false){ //todo: separate to different methods by this param. withBend
        let simpleElements = this.document.getListSimpleElements();

        let elements = simpleElements.filter(el=>{
            return (el.lineType.name=='Auto' || el.lineType.name=='Thread' ||
                (el.lineType.name=='Engrave' && !ignoreEngrave)) && el.typeName!="Text"
        });
