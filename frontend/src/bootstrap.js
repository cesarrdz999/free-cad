...
import DxfFileLoader from './file/DxfFileLoader'
...
import EngraveLineType from './model/line_types/Engrave';
...
import StraightLineWindow from "./ui/modal/StraightLine";
import QuantityWindow from "./ui/modal/Quantity";
import PropertyBar from "./ui/contracts/PropertyBar";
...
const lineTypeFactoryMethod = (config, element, createWithTool = false) => {
    ...
    if (!createWithTool) {
        if (element.elements != null) {
            let groupLineTypeName = "";
            for (let el of element.elements) {
                if (el._lineType == null || el._lineType.name == "Auto" ||
                    (groupLineTypeName != "" && el._lineType.name != groupLineTypeName)) {
                    break;
                } else {
                    groupLineTypeName = el._lineType.name;
                }
            }

            if (groupLineTypeName == "Bend") {
                return new BendLineType();
            } else if (groupLineTypeName == "Engrave") {
                return new EngraveLineType();
            } else if (groupLineTypeName == "Comment to Self") {
                return new CommentToSelfLineType();
            } else if (groupLineTypeName == "Comment to Machinist") {
                return new CommentToMachineLineType();
            }
        }
...
container.register('straightLineWindow', StraightLineWindow);
container.register('quantityWindow', QuantityWindow);
...
container.register('propertyBar', PropertyBar).singleton();
...
if (ENV != 'test') {
    ...
    let propertyBar = container.resolve('propertyBar');

    app.addHandler('selectElements', ()=>propertyBar.recalculateFields(app.selectElements));
    app.addHandler('updateSelectElements', ()=>propertyBar.recalculateFields(app.selectElements));
    app.addHandler('clearSelectElements', ()=>propertyBar.recalculateFields(app.selectElements));
}
if(ENV == 'production'){
