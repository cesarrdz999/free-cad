...
import ResizeTextQuestion from "./behaviors/ResizeTextQuestion";
/**
...
export default class ResizeElementsCommand extends ElementModificationCommand{
     ...
     * @return {{center: string, left: string, right: string}}
     */
     ...
     */
    constructor(document, elements, vector , controlPointX, controlPointY, convertCircleToSplines=false, convertTextToElements=false){
        ...
        console.log(controlPointX, controlPointY, "fg");
        ...
        /** @type {boolean} */
        this.convertTextToElements = convertTextToElements;
    ...
    executeCommand(){
        ...
        if(this._isCentralControlPoint()) {
            elements = this._convertElements();
    ...
    _convertElements(){
        ...
        for(let el of this.elements){
            if(el instanceof Arc){
                ...
                isChanged=true;
            }else if(el.typeName == "Text"){
                if(!this.convertCircleToSplines){
                    throw new Exception('You cannot perform this operation with the Text highlighted and the ' +
                        'convertTextToElements flag cleared.', this);
                }
                this._document.removeElement(el);
                el = el.convertToGeometries();
                this._document.addElement(el);
                isChanged=true;
